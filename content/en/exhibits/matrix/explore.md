...
### Interactive Demo: Visualizing Linear Transformations

Drag the matrix entries below and watch how the standard basis vectors (1,0) and (0,1) are transformed, and how the entire coordinate grid deforms. **A matrix is a deformer of space.**
...
<div style="display:flex;gap:6px;flex-wrap:wrap;margin:8px 0;">
  ...
  <button class="btn btn-sm" onclick="presetTransform(2,0,0,1)">Stretch horizontally</button>
  ...
  <button class="btn btn-sm" onclick="presetTransform(0,0,0,0)">Zero matrix</button>
  <button class="btn btn-sm" onclick="presetTransform(-1,0,0,1)">Reflect</button>
</div>

### Try It 1: A Matrix with Determinant 0

Enter $A = \begin{bmatrix}1 & 2 \\ 2 & 4\end{bmatrix}$ in the matrix above. What happens to the grid? What is det(A)?

<details>
<summary>Answer</summary>
det(A) = 1×4 − 2×2 = 0. Every point of the grid is crushed onto a single line — the plane becomes a line, and all area information is lost. Determinant 0 ⇔ the transformation is not invertible.
</details>

### Try It 2: Does a Shear Change Area?

Set the matrix to the shear $\begin{bmatrix}1 & 1 \\ 0 & 1\end{bmatrix}$ (click the "Shear" preset). What is det(A)? Did the area change?

<details>
<summary>Answer</summary>
det(A) = 1×1 − 1×0 = 1. The shear tilts the grid, yet the area is preserved (|det| = 1) — because it merely translates each row without changing the area of the unit square. A shear is an "area-preserving" deformation.
</details>

### Try It 3: Does a Rotation Change Area?

Set the matrix to the rotation $\begin{bmatrix}0 & -1 \\ 1 & 0\end{bmatrix}$ (click the "Rotate 90°" preset). What is det(A)? Did the area change?

<details>
<summary>Answer</summary>
det(A) = 0×0 − (−1)×1 = 1. Rotation preserves area (|det| = 1) and orientation (det > 0). The rotational part of a matrix never changes area — which is why "rotation + scaling" can be separated cleanly.
</details>
