<template>
  <button class="chat-fab" @click="openPanel" :aria-label="$t('chat.title')">
  ...
  <div v-if="panelOpen" class="chat-overlay" @click.self="panelOpen = false">
    <div class="chat-panel">
      ...
      <div class="chat-msgs" ref="msgList">
...
function addMsg(role, content) {
  const id = ++msgId
  messages.value.push({ id, role, content, html: renderMarkdown(content) })
  return id
}
...
async function send() {
  ...
  try {
    ...
    if (!r.ok) {
      ...
      addMsg('assistant', `**${t('chat.error')}** ${err.detail || t('chat.error')}`)
    ...
    if (e.name !== 'AbortError') {
      addMsg('assistant', `**${t('chat.error')}** ${e.message}`)
    }
  }
  if (streamText.value) {
...
header { display:flex; align-items:center; gap:8px; padding:14px 16px; border-bottom:1px solid var(--border); flex-shrink:0; }
...
.msg.hint { justify-content:center; }
