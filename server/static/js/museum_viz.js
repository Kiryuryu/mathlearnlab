...
var MuseumViz = (function() {
  ...
  function epsilonDelta(container) {
    ...
    function plot() {
      ...
      var trace1 = { x: xs, y: fx, type: 'scatter', mode: 'lines', name: 'f(x)', line: { color: '#5b7b94', width: 2 } };
      var trace2 = { x: [x0], y: [L], type: 'scatter', mode: 'markers', name: 'x=1, 极限=2', marker: { color: '#b55a5a', size: 10, symbol: 'x' } };
      // Epsilon band
      var trace3 = { x: [x0 - margin, x0 + margin], y: [L + epsilon, L + epsilon], type: 'scatter', mode: 'lines', name: 'L+ε', line: { color: 'rgba(74,124,89,0.5)', dash: 'dash' } };
      var trace4 = { x: [x0 - margin, x0 + margin], y: [L - epsilon, L - epsilon], type: 'scatter', mode: 'lines', name: 'L-ε', line: { color: 'rgba(74,124,89,0.5)', dash: 'dash' } };
      // Delta band
      var delta = epsilon; // linear function, delta = epsilon
      var trace5 = { x: [x0 - delta, x0 - delta], y: [L - epsilon - 0.5, L + epsilon + 0.5], type: 'scatter', mode: 'lines', name: 'x-δ', line: { color: 'rgba(139,115,85,0.5)', dash: 'dot' } };
      var trace6 = { x: [x0 + delta, x0 + delta], y: [L - epsilon - 0.5, L + epsilon + 0.5], type: 'scatter', mode: 'lines', name: 'x+δ', line: { color: 'rgba(139,115,85,0.5)', dash: 'dot' } };
  ...
  function tangentLine(container) {
    ...
    function plot() {
      ...
      var trace1 = { x: xs, y: ys, type: 'scatter', mode: 'lines', name: 'f(x)=x²', line: { color: '#5b7b94', width: 2 } };
      // Tangent line: y = f(a) + f'(a)(x-a)
      var tx = [a - 1.5, a + 1.5];
      var ty = [f(a) + fp(a)*(-1.5), f(a) + fp(a)*(1.5)];
      var trace2 = { x: tx, y: ty, type: 'scatter', mode: 'lines', name: '切线', line: { color: '#b55a5a', width: 2, dash: 'dash' } };
      var trace3 = { x: [a], y: [f(a)], type: 'scatter', mode: 'markers', name: '切点', marker: { color: '#b55a5a', size: 10 } };
  ...
  function riemannSum(container) {
    ...
    function plot() {
      ...
      var traces = [{ x: xs, y: ys, type: 'scatter', mode: 'lines', name: 'f(x)=x²', fill: 'tozeroy', fillcolor: 'rgba(91,123,148,0.1)', line: { color: '#5b7b94', width: 2 } }];
      var rx = [], ry = [];
      for (var i = 0; i < n; i++) {
      ...
      traces.push({ x: rx, y: ry, type: 'scatter', mode: 'lines', name: '黎曼和', fill: 'toself', fillcolor: 'rgba(139,115,85,0.3)', line: { color: '#8b7355', width: 1 } });
  ...
  function fourierSeries(container) {
    ...
    function plot() {
      ...
      var traces = [
        { x: xs, y: sw, type: 'scatter', mode: 'lines', name: 'N='+N+' 项近似', line: { color: '#5b7b94', width: 2 } },
      ];
      var harmonicsInfo = '';
  ...
  function gradientDescent(container) {
    ...
    function plot() {
      ...
      var trace2 = { x: px, y: py, z: pz, type: 'scatter3d', mode: 'lines+markers', marker: { size: 3, color: '#b55a5a' }, line: { color: '#b55a5a', width: 4 } };
